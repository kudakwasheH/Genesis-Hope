...
<div class="space-y-8 max-w-3xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
        <h3 class="text-xl font-bold text-slate-900 dark:text-white border-b border-slate-50 dark:border-slate-700 pb-3 mb-6">Write New Blog Article</h3>
        ...
        <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="E.g. Health Tips for Winter Season"
            ...
            <div class="space-y-2">
                ...
                <textarea name="content" id="content" rows="12" required placeholder="Type the awareness article content here..."
                          class="w-full rounded-2xl border-slate-200 dark:border-slate-600 dark:bg-slate-900 focus:border-teal-500 focus:ring focus:ring-teal-200 dark:text-white text-sm px-4 py-3 bg-slate-50">{{ old('content') }}</textarea>
